...
public class DmInsertExecutor extends BaseInsertExecutor implements Sequenceable {
    ...
    public Map<String,List<Object>> getPkValues() throws SQLException {
        ...
        if (isContainsPk) {
            ...
            pkValuesMap = Collections.singletonMap(columnName, getGeneratedKeys(StringUtils.EMPTY));
        } else {
            pkValuesMap = getPkValuesByColumn();
        }
        return pkValuesMap;
    ...
    public Map<String,List<Object>> getPkValuesByColumn() throws SQLException {
        ...
        if (!pkValues.isEmpty() && pkValues.get(0) instanceof SqlSequenceExpr) {
            pkValuesMap.put(pkKey, getPkValuesBySequence((SqlSequenceExpr) pkValues.get(0), StringUtils.EMPTY));
        } else if (pkValues.size() == 1 && pkValues.get(0) instanceof SqlMethodExpr) {
            pkValuesMap.put(pkKey, getGeneratedKeys(StringUtils.EMPTY));
